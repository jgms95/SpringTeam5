	...
	<div class="container" style="margin: 30px auto 30px auto;">
		...
		<ul class="pagination justify-content-center">
			<li class="page-item ${to.curPage == 1 ? 'disabled' : '' }"><a class="page-link"
					href="/event/searchlist/${to.curPage>1?to.curPage-1:1}?id=${login.id}&search=${search}">&laquo;</a></li>
			<c:forEach begin="${to.beginPageNum }" end="${to.stopPageNum }" var="page">
				<li class="page-item ${to.curPage == page ? 'active' : '' }"><a class="page-link" href="/event/searchlist/${page}?id=${login.id}&search=${search}">${page}</a></li>
			</c:forEach>
			<li class="page-item ${to.curPage == to.totalPage ? 'disabled' : '' }"><a class="page-link"
					href="/event/searchlist/${to.curPage<to.totalPage?to.curPage+1:to.curPage}?id=${login.id}&search=${search}">&raquo;</a></li>
		</ul>
		...
		<nav class="navbar navbar-expand-sm bg-light navbar-dark">
			...
			<input type="hidden" name="id" value="${login.id}">
				<aside>
					...
					<a href="/event/eventlist/1?id=${login.id}" type="button" style="background-color: rgb(67,22,7); color: white;" class="btn rounded-0">전체 목록</a>
				</aside>
			</form>
		</nav>
