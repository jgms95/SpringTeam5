...
public class NoticeController {
   ...
   public String noticelist(Model model, @PathVariable("curPage") String curPage, String id) {
	...
	public String insert(Model model, MultipartHttpServletRequest mhsq, NoticeDTO dto)
	...
	public String update(Model model, HttpSession session, int nno, String id) {
		if (session == null) {
			return "/member/login";
		} else {
			if (id.length() < 1) {
	...
	public String deletefile(String file_num, HttpServletRequest hsq) {
		...
		nService.deleteFile(file_name);

		return "success";
	}

	@RequestMapping(value = "/update", method = RequestMethod.POST)
	public String update(Model model, MultipartHttpServletRequest mhsq, NoticeDTO dto, int nno,String id)
			throws IllegalStateException, IOException {
	...
	public String searchlist(Model model, @PathVariable("curPage") String curPage, String id, String search) {
		...
		to = nService.searchlist(to,search);
		model.addAttribute("authority", authority);
		model.addAttribute("to", to);
		model.addAttribute("list", to.getList());
